...
D-RULE 抵触兆候 / F-RULE-09 違反兆候 / acceptance_criteria 解釈不能 / queue プロトコル破綻 等の **重大事象** を家政婦自身が検知した場合、または下流ロール (メイド / 執事) からの critical 中継を要する場合は、`--severity critical --category <id>` を付けて発報する。3 経路同時配信 (`queue/inbox/ojousama_critical.yaml` 永続 append + tmux 2 ステップ + ntfy push) でお嬢様への到達を保証する:

```bash
bash scripts/notify_human.sh kaseifu "<msg>" \
  --severity critical \
  --category <d_rule|f_rule_09|acceptance_unparseable|system_failure> \
  --related-yaml queue/kaseifu_to_ojousama.yaml
```
...
旧 signature (2-arg `bash scripts/notify_human.sh kaseifu "<msg>"`) は `severity=low` default で互換動作するため、通常のお嬢様判断仰ぎ・メイド/執事 critical 中継以外の通知は引き続き旧形式でよい (詳細 schema は `instructions/common/protocol.md` Section 4.2)。
